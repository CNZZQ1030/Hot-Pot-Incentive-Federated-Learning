...
class IncentiveConfig:
    ...
    # 各等级模型聚合策略的权重倍数 / Weight multipliers for each level in aggregation
    # 这些值决定了在创建分层模型时，不同等级客户端的权重
    # These values determine the weight of different level clients when creating tiered models
    TIERED_AGGREGATION_WEIGHTS = {
        'diamond': {  # 钻石级模型聚合权重 / Diamond model aggregation weights
            'diamond': 3.0,  # 钻石级客户端权重最高
            'gold': 2.0,
            'silver': 1.0,
            'bronze': 0.5
        },
        'gold': {  # 金级模型聚合权重 / Gold model aggregation weights
            'diamond': 2.0,
            'gold': 2.0,  # 金级和钻石级权重相同
            'silver': 1.0,
            'bronze': 0.5
        },
        'silver': {  # 银级模型聚合权重 / Silver model aggregation weights
            'diamond': 1.5,
            'gold': 1.5,
            'silver': 1.0,
            'bronze': 0.7
        },
        'bronze': {  # 铜级使用标准全局模型 / Bronze uses standard global model
            'diamond': 1.0,
            'gold': 1.0,
            'silver': 1.0,
            'bronze': 1.0
    ...
    # 聚合策略类型 / Aggregation strategy type
    # 'weighted': 加权聚合，高等级客户端权重更大（推荐）
    # 'strict': 严格分层，每个等级只使用该等级及以上的客户端
    TIERED_AGGREGATION_STRATEGY = 'weighted'
